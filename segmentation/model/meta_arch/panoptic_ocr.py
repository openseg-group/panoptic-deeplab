...
from segmentation.model.decoder import PanopticOCRDecoder
...
class PanopticOCR(BaseSegmentationModel):
    """
    Arguments:
        ...
        aux_semantic_loss (nn.Module): loss function
        aux_semantic_loss_weight (float): loss weight
    ...
    def __init__(self, backbone, in_channels, feature_key, low_level_channels, low_level_key,
                 ...
                 offset_loss, offset_loss_weight, aux_semantic_loss, aux_semantic_loss_weight, **kwargs):
        decoder = PanopticOCRDecoder(in_channels, feature_key, low_level_channels, low_level_key,
                                         low_level_channels_project, decoder_channels, atrous_rates, num_classes,
                                         **kwargs)
        super(PanopticOCR, self).__init__(backbone, decoder)

        self.semantic_loss = semantic_loss
        self.semantic_loss_weight = semantic_loss_weight

        # add the auxiliary loss
        self.aux_semantic_loss = aux_semantic_loss
        self.aux_semantic_loss_weight = aux_semantic_loss_weight

        self.loss_meter_dict = OrderedDict()
        self.loss_meter_dict['Loss'] = AverageMeter()
        self.loss_meter_dict['Semantic loss'] = AverageMeter()
        self.loss_meter_dict['Aux Semantic loss'] = AverageMeter()
    ...
    def loss(self, results, targets=None):
        ...
        if targets is not None:
            if 'semantic_weights' in targets.keys():
                ...
                aux_semantic_loss = self.aux_semantic_loss(
                    results['aux_semantic'], targets['semantic'], semantic_weights=targets['semantic_weights']
                ) * self.aux_semantic_loss_weight
            else:
                semantic_loss = self.semantic_loss(
                    results['semantic'], targets['semantic']) * self.semantic_loss_weight
                aux_semantic_loss = self.aux_semantic_loss(
                    results['aux_semantic'], targets['semantic']) * self.aux_semantic_loss_weight
